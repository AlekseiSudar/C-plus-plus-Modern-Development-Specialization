...
public:
    ...
    explicit ConcurrentMap(size_t bucket_count)
        : maps_(vector<MapType>(bucket_count))
        , ms_(vector<mutex>(bucket_count)){}

    WriteAccess operator[](const K& key){
        size_t index = hasher(key) % maps_.size();
        return {
            lock_guard(ms_[index]),
            maps_[index][key]
        };
    }
    ...
    bool Has(const K& key) const{
        return maps_[hasher(key) % maps_.size()].count(key);
    }
    ...
    MapType BuildOrdinaryMap() const{
        ...
        for (auto it_m = ms_.begin(), it_m_end = ms_.end(); it_m != it_m_end; it_m++, it_maps++){
            lock_guard lock(*it_m);
            ans.insert(it_maps->begin(),it_maps->end());
        }
        return ans;
    }
