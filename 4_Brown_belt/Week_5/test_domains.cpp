...
void TestSimple() {    
    ...
    const Domain to_reverse("ya.ru");
    const auto reversed = to_reverse.GetReversedParts();
    ASSERT_EQUAL(
        vector<string_view>(reversed.begin(), reversed.end()),
    ...
    ASSERT_EQUAL(
        IsSubdomain(Domain("ya.ru"),Domain("ru")),
        true
    );

    const vector<Domain> to_check({Domain("ru"), Domain("m.ru")});
    ASSERT_EQUAL(
        DomainChecker(to_check.begin(), to_check.end()).IsSubdomain(Domain("ya.ru")),
        true
    );
    
    ASSERT_EQUAL(
        CheckDomains({Domain("ru")}, {Domain("ya.ru")}),
        vector<bool>({false})
    );
